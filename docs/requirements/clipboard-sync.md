...
23. ClipboardNode must keep the authenticated device identity configurable separately from the user-visible display name.
24. ClipboardNode must clear or invalidate local login/session data when the configured authenticated device identity changes.
25. ClipboardNode must not treat publish success as remote apply success.
26. ClipboardNode must not create new MyFlowHub protocol actions or rely on server-side ClipboardNode-specific behavior.
